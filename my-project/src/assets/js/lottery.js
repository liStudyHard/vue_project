/*
 * lottery v1.0.3
 * @author blacksnail2015 2015-03-30
 *
 * modified by liubing 2017-1-18
 *
...
var lottery = {
  ...
  _setup: function() {
    ...
    for (var i = 0; i < this.options.width; ++i) {
      this.container
        .find('.lottery-group:first .lottery-unit:eq(' + i + ')')
        .attr('lottery-unit-index', i)
    }
    ...
    if (this.options.height > 1) {
      for (
        ...
        this.container
          .find('.lottery-group:last .lottery-unit:eq(' + j + ')')
          .attr('lottery-unit-index', i)
      }
    }
    ...
    for (var i = 1, len = this.options.height - 2; i <= len; ++i) {
      this.container
        .find('.lottery-group:eq(' + i + ') .lottery-unit:first')
        .attr('lottery-unit-index', lottery._count() - i)
      this.container
        .find('.lottery-group:eq(' + i + ') .lottery-unit:last')
        .attr('lottery-unit-index', this.options.width + i - 1)
    }
    this._enable()
  },
  ...
  _roll: function() {
    var _this = this
    _this.container
      .find('[lottery-unit-index=' + _this._index() + ']')
      .removeClass('active')
    ++_this.options.index
    _this.container
      .find('[lottery-unit-index=' + _this._index() + '].lottery-unit')
      .addClass('active')
  ...
  _count: function() {
    ...
    if (this.options.width < 3 || this.options.height < 3) {
      count = this.options.width * this.options.height
    } else {
      count =
