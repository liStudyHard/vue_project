 ...
 *
 * detectOs(ua)   ----检测系统类型
 * env.os.isWindowsPhone | isAndroid | isIOS,isIPhone,isIPad | version
 *
 * detectThirdApp(ua) ----检测第三方APP，目前只包含微信，微博
 * env.thirdApp.isWeixin | isWeibo
 *
 * detectTf8App(ua) ----检测淘粉吧APP
 * env.tf8App.isTf8App
 * env.tf8App.productVersion: 'IPhone_V9.74'
 * env.tf8App.version: '9.74'
 *
 * detectBrowser(ua) ----检测浏览器类型
 * env.browser.isUC | isQQ | isFirefox | (isIEMobile | isIE, isIELikeWebkit) | isChrome, isWebview | isSafari | isIOSWebview | version
 *
...
Env.prototype.detectThirdApp = function(ua = window.navigator.userAgent) {
  ...
  self.thirdApp.isWeibo = !!ua.match(/weibo/i)
}
// 检测淘粉吧APP
Env.prototype.detectTf8App = function(ua = window.navigator.userAgent) {
  let self = this
  let result = ua.match(/taofen8\/?([a-z]+_v([\d\.]+))?/i)
  if (result) {
    self.tf8App.isTf8App = true
    self.tf8App.productVersion = result[1] || ''
    self.tf8App.version = result[2] || '0'
  }
}
// 检测浏览器类型
Env.prototype.detectBrowser = function(ua = window.navigator.userAgent) {
  ...
  if ((result = ua.match(/(?:UCWEB|UCBrowser\/)([\d\.]+)/))) {
    ...
    (result = ua.match(/MSIE\s([\d\.]+)/)) ||
    (result = ua.match(/IEMobile\/([\d\.]+)/))
  ) {
    self.browser = {
      version: result[1]
    }
